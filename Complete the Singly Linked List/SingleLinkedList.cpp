...
template <class T>
ListNode<T>*  SingleLinkedList<T>::_reverse_recursive(ListNode<T>* current){
    if(current->next == nullptr){
        head->next = current;
        return current;
    }
    // _reverse_recursive(current->next)->next = current;
    _reverse_recursive(current->next);
...
void SingleLinkedList<T>::reverse_recursive(){
    ...
    _reverse_recursive(head->next)->next = nullptr;
